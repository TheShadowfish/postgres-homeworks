...
SELECT DISTINCT AVG(shipped_date - order_date) AS germany_delivery_time FROM orders WHERE ship_country = 'Germany';
...
SELECT DISTINCT MIN(unit_price) AS min_price, MAX(unit_price) AS max_price FROM products WHERE discontinued <> 1;
...
SELECT DISTINCT MIN(unit_price) AS min_price_20, MAX(unit_price) AS max_price_20 FROM products WHERE discontinued <> 1 AND units_in_stock >= 20;